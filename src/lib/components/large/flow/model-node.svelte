...
<div class="p-4 rounded bg-secondary">
  ...
  {#each model.input as input, index}
    <CustomHandle handleIO={input} handleIndex={index} handleTotal={model.input.length} position={Position.Left}/>
  {/each}

  {#each model.output as output, index}
    <CustomHandle handleIO={output} handleIndex={index} handleTotal={model.input.length} position={Position.Right}/>
  {/each}
</div>